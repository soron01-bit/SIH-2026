...
function buildFallbackResponse({ message = '', language = 'en', userLocation, activeCyclone }) {
  ...
  // Evacuation / Shelter
  if (lower.includes('shelter') || lower.includes('evacuat') || lower.includes('আশ্রয়') || lower.includes('आश्रय')) {
    if (language === 'bn') {
...
4. NEVER repeat canned or identical static sentences! Every turn must be fresh, dynamic, and directly address the user's specific words, query, or question regarding this active storm.
5. If the audio is an initial greeting or test audio, introduce yourself warmly as CycloneAI in the user's language and report current status for ${activeCyclone?.name || 'the tracked cyclone'}.
...
Respond strictly in valid JSON format:
{
  ...
  "reply": "expert, conversational response answering the user in the EXACT SAME language they spoke"
      ...
      const storm = activeCyclone?.name || userLocation?.activeCycloneName || (effectiveLang === 'bn' ? 'ঘূর্ণিঝড়' : effectiveLang === 'hi' ? 'चक्रवात' : 'the storm');
